    ...
    if game_start ==1:
        ...
        for d_m in obj_dark_missiles:
            if obj_player.collidedWith(d_m):
                player_hp -=500
        for d_b in obj_dark_balls:
            if obj_player.collidedWith(d_b):
                player_hp -=500
                        ...
                        for n in range(6):
                            if obj_satan:
                                ...
                                for s in obj_satan:
                                    ...
                                    obj_dark_missiles[len(obj_dark_missiles)-1].rotateTo(obj_dark_missiles_angle-90)
                    if dark_missile_cd ==180 *i:
                        for m in range(i):
                            obj_dark_balls.append(    Image("images\\dark_ball.png",game))
                            obj_dark_balls[len(obj_dark_balls)-1].moveTo(s.x,s.y)
                            obj_dark_balls[len(obj_dark_balls)-1].resizeBy(-60)
                    if dark_missile_cd ==210 *i:
                            obj_dark_balls[len(obj_dark_balls)-1].setSpeed(3,i)
                    if dark_missile_cd ==220 *i:
                            obj_dark_balls[len(obj_dark_balls)-1].setSpeed(5,i*10)
                    if dark_missile_cd ==230 *i:
                            obj_dark_balls[len(obj_dark_balls)-1].setSpeed(7,i*20)
                    if dark_missile_cd ==240 *i:
                            obj_dark_balls[len(obj_dark_balls)-1].moveTowards(obj_player,11)
                for d_m in obj_dark_missiles:
                    d_m.move()
                for d_b in obj_dark_balls:
                    d_b.move()
                ...
                for i in range(1000):
                    ...
                    if dark_missile_cd ==180 *i:
                        for m in range(i):
                            ...
                            obj_dark_balls[len(obj_dark_balls)-1].resizeBy(-60)
                    if dark_missile_cd ==210 *i:
                            obj_dark_balls[len(obj_dark_balls)-1].setSpeed(3,i*10)
                    if dark_missile_cd ==220 *i:
                            obj_dark_balls[len(obj_dark_balls)-1].setSpeed(5,i*20)
                    if dark_missile_cd ==230 *i:
                            obj_dark_balls[len(obj_dark_balls)-1].setSpeed(7,i*30)
                    if dark_missile_cd ==240 *i:
                            obj_dark_balls[len(obj_dark_balls)-1].moveTowards(obj_player,13)
                for d_m in obj_dark_missiles:
                    d_m.move()
                for d_b in obj_dark_balls:
                    d_b.move()
            ...
            if boss_on ==1:
                for i in range(1000):
                    ...
                    if dark_missile_cd ==230 *i:
                            obj_dark_balls[len(obj_dark_balls)-1].setSpeed(7,i*30)
                    if dark_missile_cd ==240 *i:
                            obj_dark_balls[len(obj_dark_balls)-1].moveTowards(obj_player,15)
                for d_m in obj_dark_missiles:
                    d_m.move()
                for d_b in obj_dark_balls:
                    d_b.move()
