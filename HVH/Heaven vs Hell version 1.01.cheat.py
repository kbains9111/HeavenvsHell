...
while not game.over: 
    ...
    if game_start ==0:
        title_bg.draw()
        game.drawText("Press [A] -Easy mode.",game.width/2-160,game.height/2-120,Font(green,38,yellow))
        game.drawText("Press [S] -Hard mode. ",game.width/2-160,game.height/2+120,Font(red,38,yellow))
        if keys.Pressed[K_a]:
            difficulty =1
            game_start =1
        if keys.Pressed[K_s]:
    ...
    if game_start ==1:
        ...
        game.drawText("15135: " +str(difficulty),800,700,Font(black,50,red))
        if level  ==1:
