...
while not game.over: 
    ...
    if game_start ==0:
        ...
        game.drawText("Press [D] -Hell mode. ",game.width/2,game.height/2,Font(magenta,38,black))
        if keys.Pressed[K_a]:
            difficulty =1
            game_start =1
        if keys.Pressed[K_s]:
    ...
    if game_start ==1:
        ...
        if level  ==1:
            ...
            game.drawText("Upgrade[NUMBER](COST)",20,110,Font(red,50,blue))
        for s in obj_satan:
            if boss_stage ==1:
...
if ending ==1:
    end_1_bg.draw()
    game.drawText("Satan destroyed Heaven....",game.width/2 -250,game.height/2-150,Font(magenta,60,black))
if ending ==2:
    end_2_bg.draw()
    game.drawText("You protected Heaven !!",game.width/2 -250,game.height/2-150,Font(cyan,60,magenta))
...
난이도는 데빌모드로 합니다.
굉장히 어렵겠지만 / 노력해봅시다.
With using obj_dark_misslie List
